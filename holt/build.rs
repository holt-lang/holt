//! Embeds the `stdlib/` sources into the `holt` binary so `holt setup` can
//! install them to `~/.hella/lib` on machines without a repo checkout.
//! The file list is generated (sorted `**/*.hlt` under `../stdlib`), so
...
fn main() {
    ...
    collect_hlt(&stdlib, &stdlib, &mut rels);
    rels.sort();

    let mut out = String::from(
        "/// Embedded standard-library sources: `(relative path, contents)`.\n\
         /// Generated by `holt/build.rs` from `stdlib/**/*.hlt`.\n\
...
fn collect_hlt(root: &PathBuf, dir: &PathBuf, out: &mut Vec<String>) {
