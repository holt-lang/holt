...
* Resolution (`compiler/src/modules.rs`, shared by CLI and LSP), in order:
  1. **Project root** — nearest ancestor containing `main.hlt` (else the
     entry's own directory). `import foo` → `foo.hlt` (else `foo/mod.hlt`);
     `import net::http` → `net/http.hlt`. Cycles cut by visited-file set.
  2. **Dev checkout** — ancestor `stdlib/` (repo work uses live sources).
  3. **`~/.hella/lib`** (UNIX) — populated by `holt setup` (embedded
     `stdlib/**/*.hlt`, list auto-generated by `holt/build.rs`).
...
4. Add `examples/stdlib_<mod>.hlt` that `import std::<mod>` and is compiled+linked as regression.
5. Keep `stdlib/README.md` symbol index in sync.

## Testing

- Unit: `cargo run -- <example>.hlt` → `./example.out` and check stdout.
- Example: `examples/stdlib_io.hlt` imports `std::io` and calls each shipped function.
