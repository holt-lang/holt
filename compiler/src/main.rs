...
fn main() -> miette::Result<()> {
    ...
    if !out.errors.is_empty() {
        let parts: Vec<(crate::token::Span, String)> =
            out.errors.into_iter().map(|e| (e.span, format!("unexpected token `{}`", e.slice))).collect();
        let multi = error::MultiDiagnostic::from_errors(filename.clone(), source.clone(), parts, "lexing failed".into());
        eprintln!("{:?}", Report::new(multi));
        std::process::exit(1);
    }

    // Phase 0 compatibility: empty file (only trivia/newlines) is lex-ok even when compiling
    let is_empty_program = out.tokens.iter().all(|st| matches!(st.token, token::Token::Newline | token::Token::Semicolon));
    if is_empty_program {
        if args.lex {
    ...
    if args.lex {
        for st in &out.tokens {
            ...
            let display_slice = if st.token == token::Token::Newline { "\\n".to_string() } else { slice.replace('\n', "\\n") };
            if args.show_spans {
                println!("{:>4}..{:<4}  {:18}  `{}`", st.span.start, st.span.end, format!("{}", st.token), display_slice);
            } else {
                println!("{:18}  `{}`  @{}..{}", format!("{}", st.token), display_slice, st.span.start, st.span.end);
            }
        }
    ...
    let program = match parse::parse(out.tokens.clone(), source.clone()) {
        Ok(p) => p,
        Err(e) => {
            let diag = error::SingleDiagnostic::new(filename.clone(), source.clone(), e.span, e.message);
    ...
    if !sema_errors.is_empty() {
        let parts: Vec<(crate::token::Span, String)> = sema_errors.into_iter().map(|e| (e.span, e.message)).collect();
        let multi = error::MultiDiagnostic::from_errors(filename.clone(), source.clone(), parts, "semantic error".into());
        eprintln!("{:?}", Report::new(multi));
        std::process::exit(1);
    }
    ...
    let obj_path = if let Some(o) = &args.output {
        ...
        let mut p = o.clone(); p.set_extension("o"); p
    } else {
        let mut p = args.file.clone(); p.set_extension("o"); p
    };
    ...
    let exe_path = args.output.clone().unwrap_or_else(|| {
        let mut p = args.file.clone(); p.set_extension(""); let name = p.file_name().unwrap().to_string_lossy().to_string() + ".out"; p.with_file_name(name)
    });
    ...
    eprintln!("ok: {} → {} (linked)", args.file.display(), exe_path.display());
    Ok(())
}
...
fn generate_ir_string(program: &ast::Program) -> miette::Result<String> {
    ...
    cg.compile_program(program).map_err(|e| miette::miette!("codegen error: {} at {}..{}", e.message, e.span.start, e.span.end))?;
    Ok(cg.get_module_ir())
}

fn codegen_to_object(program: &ast::Program, obj_path: &Path, filename: &str, source: &str) -> miette::Result<()> {
    if let Err(msg) = codegen::compile_to_object(program, obj_path) {
        let diag = error::SingleDiagnostic::new(filename.to_string(), source.to_string(), crate::token::Span::new(0, source.len()), msg);
        eprintln!("{:?}", Report::new(diag));
        std::process::exit(1);
    }
    Ok(())
