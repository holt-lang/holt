...
impl<'ctx> Codegen<'ctx> {
    pub fn new(context: &'ctx Context, module_name: &str) -> Self {
        ...
        Self { context, module, builder, vars: Vec::new(), funcs: HashMap::new(), struct_types: HashMap::new(), struct_fields: HashMap::new(), cur_fn: None, cur_is_main: false }
    }
    ...
    fn declare_struct(&mut self, s: &StructDecl) -> Result<(), CodegenError> {
        if self.struct_types.contains_key(&s.name) {
            return Err(CodegenError{message: format!("duplicate struct {}", s.name), span: s.name_span});
        }
    ...
    fn llvm_ty_for(&self, ty: &Type) -> BasicTypeEnum<'ctx> {
        match ty {
            ...
            Type::Named(n, _) => {
                if let Some(st) = self.struct_types.get(n) { st.as_basic_type_enum().into() } else {
    ...
    fn llvm_ty_for_sema(&self, ty: &crate::sema::Ty) -> Option<BasicTypeEnum<'ctx>> {
        match ty {
            ...
            crate::sema::Ty::Struct(n) => {
                let st = self.struct_types.get(n).unwrap_or_else(|| panic!("unknown struct {n} in llvm_ty_for_sema"));
        ...
        let fn_ty = if f.name == "main" {
            ...
            match ret_sema {
                ...
                crate::sema::Ty::Struct(ref n) => {
                    let st = self.struct_types.get(n).ok_or(CodegenError{message: format!("unknown struct {n}"), span: f.ret_ty.span()})?;
    ...
    fn codegen_expr(&mut self, expr: &Expr) -> Result<BasicValueEnum<'ctx>, CodegenError> {
        match &expr.kind {
            ...
            ExprKind::Unary{op, expr: inner} => {
                ...
                match op {
                    UnaryOp::Not => {
                        ...
                        Ok(self.builder.build_xor(b, self.context.bool_type().const_int(1, false), "not").unwrap().into())
                    }
                    UnaryOp::Neg => {
                        let i = v.into_int_value();
                        Ok(self.builder.build_int_sub(self.context.i64_type().const_int(0, false), i, "neg").unwrap().into())
                    }
                    UnaryOp::Pos => Ok(v),
                }
            }
            ...
            ExprKind::Assign{lhs, value} => {
                ...
                // lvalue may be Ident or MemberAccess
                match &lhs.kind {
                    ExprKind::Ident(name) => {
                        let (ptr, _) = self.lookup_var(name).ok_or(CodegenError{message: format!("undefined var {name}"), span: lhs.span})?;
                        self.builder.build_store(ptr, val).unwrap();
                        Ok(val)
                    }
                    ExprKind::MemberAccess{object, field, field_span: _} => {
                        // field assignment: object.field = value
                        let field_ptr = self.codegen_field_ptr(object, field)?;
                        self.builder.build_store(field_ptr, val).unwrap();
                        Ok(val)
                    }
                    _ => Err(CodegenError{message: "invalid assignment target".into(), span: lhs.span}),
                }
            }
            ...
            ExprKind::MemberAccess{object, field, field_span: _} => {
                ...
                let field_ptr = self.codegen_field_ptr(object, field)?;
                // Determine field type via struct field map → need to load with type
                // Derive from struct type: we can look up struct field's LLVM type via GEP type
                // Simplify: infer load type via the field's stored type: we have struct type, get field index, then field type
                // We'll retrieve it: find struct name from object's type (via lookup)
                // Quick: try to infer via loaded field ptr type: we need to know field's BasicTypeEnum
                // We'll look up struct info then field ty
                let obj_ty = self.infer_expr_ty(object)?; // helper to get Ty
                if let crate::sema::Ty::Struct(ref sname) = obj_ty {
                    ...
                    Err(CodegenError{message: format!("field access on non-struct"), span: expr.span})
                }
            }
            ExprKind::StructLit{ty, fields} => {
                let sname = match ty {
                    Type::Named(n, _) => n.clone(),
                    _ => return Err(CodegenError{message: "struct literal requires named type".into(), span: expr.span}),
                };
                let st = *self.struct_types.get(&sname).ok_or(CodegenError{message: format!("unknown struct {sname}"), span: expr.span})?;
                let field_map = self.struct_fields.get(&sname).unwrap().clone();
                // Allocate temp struct on stack, fill fields, load aggregate value
                let tmp = self.builder.build_alloca(st, "struct.lit.tmp").unwrap();
                for (fname, _fspan, fexpr) in fields {
                    let idx = *field_map.get(fname).ok_or(CodegenError{message: format!("unknown field {fname}"), span: expr.span})?;
                    let val = self.codegen_expr(fexpr)?;
                    let field_ptr = self.builder.build_struct_gep(st, tmp, idx, &format!("s.{}", fname)).unwrap();
                    self.builder.build_store(field_ptr, val).unwrap();
                }
                let loaded = self.builder.build_load(st.as_basic_type_enum(), tmp, "struct.lit").unwrap();
                Ok(loaded)
            }
        }
    }
    ...
    fn codegen_field_ptr(&self, object: &Expr, field: &str) -> Result<PointerValue<'ctx>, CodegenError> {
        ...
        let (base_ptr, base_struct_name) = self.resolve_base_struct_ptr(object)?;
        let fields = self.struct_fields.get(&base_struct_name).ok_or(CodegenError{message: format!("unknown struct {base_struct_name}"), span: object.span})?;
        let field_idx = *fields.get(field).ok_or(CodegenError{message: format!("struct {base_struct_name} has no field {field}"), span: object.span})?;
        ...
        if let ExprKind::MemberAccess{object: inner, field: inner_field, ..} = &object.kind {
            ...
            if let crate::sema::Ty::Struct(ref inner_sname) = inner_lval_ty {
                ...
                let parent_fields = self.struct_fields.get(inner_sname).unwrap();
                ...
                let ptr = self.builder.build_struct_gep(*inner_st, inner_ptr, idx2, field).unwrap();
                return Ok(ptr);
            } else {
                return Err(CodegenError{message: "nested field access on non-struct".into(), span: object.span});
            }
        }
        // Simple case: base ident field
        let ptr = self.builder.build_struct_gep(*st, base_ptr, field_idx, field).unwrap();
        Ok(ptr)
    }

    fn resolve_base_struct_ptr(&self, object: &Expr) -> Result<(PointerValue<'ctx>, String), CodegenError> {
        match &object.kind {
            ExprKind::Ident(name) => {
            ...
            ExprKind::MemberAccess{object: inner, field, ..} => {
                ...
                if let crate::sema::Ty::Struct(ref n) = inner_ty {
                    ...
                    Err(CodegenError{message: "resolve base not struct".into(), span: object.span})
                }
            }
            _ => Err(CodegenError{message: "field access base must be variable or field".into(), span: object.span}),
        }
    }

    fn ty_to_struct_name(&self, ty: &BasicTypeEnum<'ctx>) -> Result<String, CodegenError> {
        if let BasicTypeEnum::StructType(st) = ty {
            for (name, s) in &self.struct_types {
                if *s == *st { return Ok(name.clone()); }
                if s.as_basic_type_enum() == *ty { return Ok(name.clone()); }
            }
            Err(CodegenError{message: "struct type not found".into(), span: Span::new(0,0)})
        } else {
            Err(CodegenError{message: "variable is not struct".into(), span: Span::new(0,0)})
        }
    }

    fn infer_expr_ty(&self, expr: &Expr) -> Result<crate::sema::Ty, CodegenError> {
